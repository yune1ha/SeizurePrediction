...
import random, copy, warnings, math, time, csv, pickle
...
#########################################################################################
# Models ### CHECKER
engine_names = ('lstm_1',) #('CNN1Db2', 'CNN2Db1', 'BSDCNNb1')
engines = (lstm_1,) #(cnn_lstm_1, cnn2d, bsdcnn)
...
#ch_picks_list += all_ch ### CHECKER

# Global Settings
learning_rate = 1e-5 
batch_size = 256 # CHECKER (16 if cnn2d, 256 if lstm)
epochs = 1#10** 2  # CHECKER
patience = 42
...
def modeler(arr_name, patients, all_preictals, all_interictals, t, cw, run_name, run_dir, engine_name, engine, ch_picks):
    ...
    for idx, patient in enumerate(patients[:]):
        try:
            ...
            # for train_index, val_index in skf.split(np.zeros(len(train_data)),y_train):
            ...
            #     model_name = f'./Outputs/models/best-model-kfold-{fold}.hdf5'
            ...
            # Fit model
            hist = model.fit(train_data, ##train iterator
                  epochs=epochs,
                  verbose=1,
                  validation_data=val_data, ##validation_iterator
            ...
            # Save 
            model.save_weights(os.path.join(run_dir, run_name+".hdf5"))
            
            with open(os.path.join(run_dir, 'trainHistoryDict'), 'wb') as file_pi:
                pickle.dump(hist.history, file_pi)
            
            # Metrics
            val_loss, val_acc, *is_anything_else_being_returned = model.evaluate(X_valid, y_valid, verbose=1, batch_size=batch_size)
            ...
            print('auc() == roc_auc_score', auc_keras == auc_score, file=t)
            print('AUC Score:', round(auc_keras,4))
...
#             plt.clf()
            ...
            # plt.plot(fpr_keras, tpr_keras, marker='.', label='{} (auc = {})'.format(patient, round(auc_keras,4)) )
...
def ignition(arr_name, run_name, run_dir, engine_name, engine, ch_picks):
    ...
    cw = csv.writer(c)
    header = ['patient', 'auc', 'acc', 'spec', 'sens', 'fpr', 'loss']
    cw.writerow(header)
